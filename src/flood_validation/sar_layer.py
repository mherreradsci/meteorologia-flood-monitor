...
from flood_monitor import (EPOCH, detect_flood, permanent_water_mask,
...
def write_geotiff_geojson(template, flood: np.ndarray, output_dir: Path,
                          tag: str) -> dict[str, Path | None]:
    """Escribe el GeoTIFF binario y el GeoJSON vectorizado de la capa S1.

    Mismo método que `flood_monitor.save_outputs` (rasterio.features.shapes
    + reproyección a EPSG:4326), pero sin el quicklook PNG ni el mapa HTML:
    esos llegan con report.py en la Fase 6, cuando hay fusión de sensores
    que mostrar y no solo esta capa intermedia.
    """
    ...
    tif = output_dir / f"real_flood_s1_{tag}.tif"
    ...
    if geoms:
        ...
        geojson = output_dir / f"real_flood_s1_{tag}.geojson"
        gdf.to_file(geojson, driver="GeoJSON")

    print(f"[+] Sentinel-1: GeoTIFF {tif}" +
